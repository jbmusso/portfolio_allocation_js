...
function compositionsIterator_(n, k, useArrayCopy) {
  ...
  this.mtc = false;
  this.r =
    typeof Uint32Array === 'function' ? new Uint32Array(k) : new Array(k);
  this.t = this.n;
  this.h = 0;
...
function randomKSubsetIterator_(n, k, useArrayCopy) {
  ...
  this.method_a = function () {
    ...
    if (S === this.N) {
      // the out of range value S = N must never be generated (which could be in case of roundoff errors)
      S = this.N - 1;
    }
  ...
  this.method_d = function () {
    ...
    while (this.nn > 1 && threshold < this.N) {
      ...
      while (true) {
        ...
        if (this.N / (-X + this.N) >= y1 * Math.pow(y2, nmin1inv)) {
          // Math.exp(Math.log(a) * b) === Math.pow(a, b)
          // Accept
...
function subsetsIterator_(n) {
  ...
  this.next = function () {
    ...
    if (this.firstcall) {
      ...
      nextSubset =
        typeof Uint32Array === 'function'
          ? new Uint32Array(this.ncard)
          : new Array(this.ncard);
      var idx = 0;
      for (var i = 0; i <= this.n - 1; ++i) {
        if (this.iin[i] == 1) {
          nextSubset[idx++] = i + 1;
