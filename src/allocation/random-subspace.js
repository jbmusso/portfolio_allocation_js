...
self.randomSubspaceOptimizationWeights = function (nbAssets, fct, opt) {
  ...
  if (sizeSubsets <= 1 || sizeSubsets >= nbAssets + 1) {
    throw new Error(
      'the size of the subsets of assets must be between 2 and ' +
        nbAssets.toString()
    );
  }
  ...
  if (subsetsGenerationMethod === 'random') {
    subsetAssetsIdxIterator = new randomKSubsetIterator_(
      nbAssets,
      sizeSubsets,
      false
    ); // use no array copy in the subsets generation to improve performances
  } else if (subsetsGenerationMethod === 'deterministic') {
    subsetAssetsIdxIterator = new kSubsetsIterator_(
      nbAssets,
      sizeSubsets,
      false
    ); // use no array copy in the subsets generation to improve performances
  ...
  var subsetPortfolioOptimizationMethodParams =
    opt.subsetPortfolioOptimizationMethodParams;
  ...
  if (opt.constraints.minWeights) {
    subsetPortfolioOptimizationMethodParams.constraints.minWeights =
      typeof Float64Array === 'function'
        ? new Float64Array(sizeSubsets)
        : new Array(sizeSubsets);
  }
  if (opt.constraints.maxWeights) {
    subsetPortfolioOptimizationMethodParams.constraints.maxWeights =
      typeof Float64Array === 'function'
        ? new Float64Array(sizeSubsets)
        : new Array(sizeSubsets);
  }
  ...
  for (var k = 0; k < nbSubsets; ++k) {
    ...
    if (opt.constraints.minWeights) {
      for (var i = 0; i < sizeSubsets; ++i) {
        subsetPortfolioOptimizationMethodParams.constraints.minWeights[i] =
          opt.constraints.minWeights[subsetAssetsIdx[i] - 1];
      }
    }
    if (opt.constraints.maxWeights) {
      for (var i = 0; i < sizeSubsets; ++i) {
        subsetPortfolioOptimizationMethodParams.constraints.maxWeights[i] =
          opt.constraints.maxWeights[subsetAssetsIdx[i] - 1];
      }
    }
    ...
    try {
      ...
      if (subsetWeights.length != sizeSubsets) {
        throw new Error(
          'internal error: the portfolio optimization method did not return the expected number of weights'
        );
      }
    } catch (e) {
  ...
  if (nbFeasibleGeneratedPortfolios === 0) {
    ...
    var nbInfeasibleGeneratedPortfolios =
      nbSubsets - nbFeasibleGeneratedPortfolios;

    if (
      nbInfeasibleGeneratedPortfolios >= 1 &&
      nbInfeasibleGeneratedPortfolios / nbSubsets >= maxInfeasibleSubsetsRatio
    ) {
...
self.randomSubspaceMeanVarianceOptimizationWeights = function (mu, sigma, opt) {
  ...
  opt.subsetPortfolioOptimizationMethodParams.subsetMu = Matrix_.zeros(
    opt.sizeSubsets,
    1
  );
  opt.subsetPortfolioOptimizationMethodParams.subsetSigma = Matrix_.zeros(
    opt.sizeSubsets,
    opt.sizeSubsets
  );

  // Definition of the portfolio optimization algorithm to use on the subsets
  function subsetMeanVarianceOptimization(
    subsetAssetsIdx,
    subsetPortfolioOptimizationMethodParams
  ) {
    // Extract the returns of the selected assets
    var subsetMu = mu.submatrix(
      subsetAssetsIdx,
      [1],
      subsetPortfolioOptimizationMethodParams.subsetMu
    );

    // Extract the covariance matrix of the selected assets
    var subsetSigma = sigma.submatrix(
      subsetAssetsIdx,
      subsetAssetsIdx,
      subsetPortfolioOptimizationMethodParams.subsetSigma
    );

    // Return the weights of the mean-variance optimal portfolio of the selected assets
    return self.meanVarianceOptimizationWeights(
      subsetMu,
      subsetSigma,
      subsetPortfolioOptimizationMethodParams
    );
  ...
  return self.randomSubspaceOptimizationWeights(
    nbAssets,
    subsetMeanVarianceOptimization,
    opt
  );
};
