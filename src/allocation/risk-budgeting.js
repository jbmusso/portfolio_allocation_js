...
self.riskBudgetingWeights = function (sigma, rb, opt) {
  ...
  var x = Matrix_.fill(nbAssets, 1, function (i, j) {
    return 1 / nbAssets;
  });
  ...
  var obj =
    ...
    Matrix_.vectorDotProduct(
      rb,
      x.elemMap(function (i, j, val) {
        return Math.log(val);
      })
    ); // the objective function to be minimized, c.f. formula 3 of the second reference
  ...
  while (!converged) {
    ...
    for (var i = 1; i <= nbAssets; ++i) {
      ...
      if (disc < 0) {
        throw new Error(
          'Negative discriminant during iteration ' +
            iter +
            ', covariance matrix might not be semi-definite positive'
        );
      }
      ...
      for (var j = 1; j <= nbAssets; ++j) {
        sigma_x.data[j - 1] +=
          sigma.data[(j - 1) * sigma.nbColumns + (i - 1)] * xi_star;
        sigma_x.data[j - 1] -=
          sigma.data[(j - 1) * sigma.nbColumns + (i - 1)] * xi_old;
      }
      ...
      if (sigma_x_x < 0) {
        if (-sigma_x_x < eps_tol) {
          ...
          throw new Error(
            'Negative volatility during iteration ' +
              iter +
              ', covariance matrix might not be semi-definite positive'
          );
        }
