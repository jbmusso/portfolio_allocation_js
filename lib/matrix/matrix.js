...
function Matrix_(input) {
  ...
  if (input instanceof Array && input[0] instanceof Array) {
    ...
    (typeof Float64Array === 'function' && input instanceof Float64Array)
  ) {
    // Simplified constructor for a column matrix (i.e., a vector)
    return fromArray(input);
  } else if (input instanceof Matrix_) {
    // Equivalent to a "clone" operation on the Matrix
  ...
  if (out === undefined) {
    ...
    obj.data =
      typeof Float64Array === 'function'
        ? new Float64Array(obj.nbRows * obj.nbColumns)
        : new Array(obj.nbRows * obj.nbColumns);
  } else if (!(out instanceof Matrix_)) {
    throw new Error('provided output must be a matrix');
  } else if (out.nbRows !== n || out.nbColumns !== m) {
    throw new Error(
      'provided output matrix size does not match expected matrix size: ' +
        '(' +
        out.nbRows +
        ',' +
        out.nbColumns +
...
Matrix_.prototype = {
  ...
  toString: function () {
    ...
    for (var i = 0; i < this.nbRows; ++i) {
      ...
      for (var j = 0; j < this.nbColumns; ++j) {
        ...
        strMat.push(
          new Array(maxLen - strVal.length + 1).join(' ') + strVal + ' '
        );
      }
  ...
  getValueAt: function (i, j) {
    ...
    if (i < 1 || j < 1 || i > this.nbRows || j > this.nbColumns) {
      throw new Error(
        'index out of bounds when getting matrix value, (' +
          i +
          ',' +
          j +
          ') in size (' +
  ...
  row: function (i, out) {
    ...
    if (i < 1 || i > this.nbRows) {
      throw new Error(
        'index out of bounds when getting matrix row, (' +
          i +
          ') in size (' +
          this.nbRows +
          ',' +
  ...
  elemMap: function (fct, out) {
    ...
    for (var i = 0; i < obj.nbRows; ++i) {
      for (var j = 0; j < obj.nbColumns; ++j) {
        obj.data[i * obj.nbColumns + j] = fct(
          i + 1,
          j + 1,
          this.data[i * this.nbColumns + j]
        );
      }
    }
  ...
  submatrix: function (rindexes, cindexes, out) {
    ...
    if (
      !(rindexes instanceof Array || rindexes instanceof Uint32Array) ||
      rindexes.length == 0
    ) {
      throw new Error('first parameter must be a non empty array');
    }
    if (
      !(cindexes instanceof Array || cindexes instanceof Uint32Array) ||
      cindexes.length == 0
    ) {
    ...
    for (var i = 0; i < obj.nbRows; ++i) {
      ...
      for (var j = 0; j < obj.nbColumns; ++j) {
        ...
        obj.data[i * obj.nbColumns + j] = this.data[
          rindex * this.nbColumns + cindex
        ];
      }
    }
...
Matrix_.xpy = function (X, Y, out) {
  ...
  if (X.nbColumns !== Y.nbColumns || X.nbRows !== Y.nbRows) {
    throw new Error(
      'input matrices sizes do not match: ' +
        '(' +
        X.nbRows +
        ',' +
        X.nbColumns +
...
Matrix_.elementwiseProduct = function (X, Y, out) {
  ...
  if (
    !(
      (X.nbRows === Y.nbRows && X.nbColumns === Y.nbColumns) ||
  ...
  if (X.nbRows === Y.nbRows && X.nbColumns === Y.nbColumns) {
    // full matrix elementwise product
    var nbElements = nbRows * nbColumns;
    for (var k = 0; k < nbElements; ++k) {
      obj.data[k] = X.data[k] * Y.data[k];
    }
  } else if (Y.nbRows === X.nbRows && Y.nbColumns === 1) {
    // row matrix elementwise product
    var x_ij_idx = 0;
  ...
  } else if (Y.nbRows === 1 && Y.nbColumns === X.nbColumns) {
    // column matrix elementwise product
    var x_ij_idx = 0;
    for (var i = 0; i < nbRows; ++i) {
      for (var j = 0; j < nbColumns; ++j) {
...
Matrix_.ax = function (a, X, out) {
  ...
  for (var i = 1; i <= n; ++i) {
    for (var j = 1; j <= m; ++j) {
      obj.setValue(i, j, a * X.getValue(i, j));
    }
  }
  ...
  for (var i = 0; i < X.nbRows; ++i) {
    for (var j = 0; j < Y.nbRows; ++j) {
      ...
      for (var k = 0; k < X.nbColumns; ++k) {
        obj.data[i * obj.nbColumns + j] +=
          X.data[i * X.nbColumns + k] * Y.data[j * Y.nbColumns + k];
      }
...
Matrix_.svdDecomposition = function (A, opt) {
  ...
  if (A.nbRows < A.nbColumns) {
    throw new Error(
      'matrix has more columns than rows: ' + A.nbColumns + ' v.s. ' + A.nbRows
    );
  }
  ...
  var iter = 0;
  var u_columns_two_norm_sq =
    typeof Float64Array === 'function' ? new Float64Array(n) : new Array(n);
  while (true) {
    ...
    for (var j = 2; j <= n; ++j) {
      for (var i = 1; i <= j - 1; ++i) {
        ...
        for (var k = 1; k <= m; ++k) {
          cc +=
            uu.data[(k - 1) * uu.nbColumns + (i - 1)] *
            uu.data[(k - 1) * uu.nbColumns + (j - 1)];
        }
        ...
        // - ||U(:,i)||_2 > eps * ||U||_f
        // - ||U(:,j)||_2 > eps * ||U||_f
        if (Math.abs(cc) <= eps * Math.sqrt(m * aa * bb)) {
          // this condition also covers the case cc = 0, which would make zeta below indefinite
          continue;
        }
        if (
          Math.sqrt(aa) <= eps * u_frob_norm ||
          Math.sqrt(bb) <= eps * u_frob_norm
        ) {
          continue;
        }
        ...
        var zeta = (aa - bb) / (2 * cc);
        var t =
          (zeta >= 0 ? 1 : -1) / (Math.abs(zeta) + Math.sqrt(1 + zeta * zeta)); // first part emulates sign(zeta)
  ...
  var sigmas =
    typeof Float64Array === 'function' ? new Float64Array(n) : new Array(n);
  var sigmas_idx =
    typeof Uint32Array === 'function' ? new Uint32Array(n) : new Array(n);
  for (var j = 1; j <= n; ++j) {
    // Compute sigma_j
  ...
  sigmas_idx.sort(function (a, b) {
    return sigmas[b - 1] - sigmas[a - 1];
  });
  ...
  for (var j = 1; j <= n; ++j) {
    ...
    if (sigma_j != 0) {
      for (var i = 1; i <= m; ++i) {
        uuu.data[(i - 1) * uuu.nbColumns + (j - 1)] =
          uu.data[(i - 1) * uu.nbColumns + (sigma_j_col_idx - 1)] / sigma_j;
      }
    }

    // Thin V(:,j) must be equal to the column of the intermediate V corresponding to sigma_j
    for (var i = 1; i <= n; ++i) {
      vvv.data[(i - 1) * vvv.nbColumns + (j - 1)] =
        vv.data[(i - 1) * vv.nbColumns + (sigma_j_col_idx - 1)];
    }
  }
  ...
  for (var j = 1; j <= n; ++j) {
    ...
    if (sigma_j != 0) {
      for (var i = 1; i <= m; ++i) {
        uuuu.data[(i - 1) * uuuu.nbColumns + (j - 1)] =
          uuu.data[(i - 1) * uuu.nbColumns + (j - 1)];
      }
    } else {
      for (var i = 1; i <= m; ++i) {
        uuuu.data[(i - 1) * uuuu.nbColumns + (j - 1)] =
          q.data[(i - 1) * q.nbColumns + (j - 1)];
      }
    }
  }
  for (var j = n + 1; j <= m; ++j) {
...
Matrix_.nullSpace = function (A, opt) {
  ...
  if (m >= n) {
    ...
    if (p == n) {
      ...
      for (var j = p + 1; j <= n; ++j) {
        for (var i = 1; i <= n; ++i) {
          a_ns.data[(i - 1) * a_ns.nbColumns + (j - (p + 1) + 1 - 1)] =
            v.data[(i - 1) * v.nbColumns + (j - 1)];
        }
      }
    }
  } else {
    // Compute a thin SVD decomposition of A^t: A^t = U*S*V^t
    var svd = Matrix_.svdDecomposition(A.transpose(), {
      maxIter: -1,
      svdForm: 'full',
    });
    var u = svd[0];
    ...
    if (p == m) {
      ...
      for (var j = 1; j <= p; ++j) {
        for (var i = 1; i <= n; ++i) {
          ta_rg.data[(i - 1) * ta_rg.nbColumns + (j - 1)] =
            u.data[(i - 1) * u.nbColumns + (j - 1)];
        }
      }
...
Matrix_.linsolveBackSubstitution = function (A, b, out) {
  ...
  if (!A.isSquare()) {
    throw new Error(
      'matrix is not square: ' + '(' + A.nbRows + ',' + A.nbColumns + ')'
    );
  }
  if (A.nbRows !== b.nbRows) {
    throw new Error(
      'matrix and second member sizes do not match: ' +
        '(' +
        A.nbRows +
        ',' +
        A.nbColumns +
  ...
  if (b.nbColumns !== 1) {
    throw new Error(
      'b is not a vector: ' + '(' + b.nbRows + ',' + b.nbColumns + ')'
    );
  }
  ...
  for (var i = n; i >= 1; --i) {
    ...
    for (var j = i + 1; j <= n; ++j) {
      x.data[i - 1] =
        x.data[i - 1] - A.data[(i - 1) * A.nbColumns + (j - 1)] * x.data[j - 1];
    }
    var a_ii = A.data[(i - 1) * A.nbColumns + (i - 1)];
    if (a_ii == 0) {
      throw new Error(
        'input matrix is not invertible: zero diagonal coefficient at index ' +
          i
      );
    } else {
...
Matrix_.linsolveExtendedKaczmarz = function (A, b, opt) {
  ...
  var a_rows_two_norm_sq =
    typeof Float64Array === 'function' ? new Float64Array(m) : new Array(m);
  for (var i = 1; i <= m; ++i) {
  ...
  if (randomized == false) {
    ...
    while (true) {
      ...
      if (maxIterations !== -1 && iter >= maxIterations) {
        throw new Error(
          'maximum number of iterations reached: ' + maxIterations
        );
      }
      ...
      for (var i = 1; i <= m; ++i) {
        ...
        for (var j = 1; j <= n; ++j) {
          a_i_x_k +=
            A.data[(i - 1) * A.nbColumns + (j - 1)] *
            x_k.data[(j - 1) * x_k.nbColumns];
        }
        var r_k =
          a_i_x_k -
          (b.data[(i - 1) * b.nbColumns + 0] -
            z_k.data[(i - 1) * z_k.nbColumns + 0]);

        // Update x_k: x_k+1 = x_k - r_k/||A(i,:)||_2^2 * A(i,:)
        for (var j = 1; j <= n; ++j) {
          x_k.data[(j - 1) * x_k.nbColumns] -=
            (r_k / a_rows_two_norm_sq[i - 1]) *
            A.data[(i - 1) * A.nbColumns + (j - 1)];
        }
      }
      ...
      for (var j = 1; j <= n; ++j) {
        ...
        for (var i = 1; i <= m; ++i) {
          z_k.data[(i - 1) * z_k.nbColumns + 0] -=
            (a_j_z_k / a_columns_two_norm_sq[j - 1]) *
            A.data[(i - 1) * A.nbColumns + (j - 1)];
        }
      }
  ...
  // Randomized Extended Kaczmarz, c.f. algorithm 3 of the second reference.
    ...
    while (true) {
      ...
      if ((iter % 8) * Math.min(m, n) === 0) {
        ...
        if (
          x_res_two_norm <= eps * a_frob_norm * x_k_two_norm &&
