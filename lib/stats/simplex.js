...
function simplexEmptinessCheck_(n, l, u) {
  ...
  for (var i = 0; i < n; ++i) {
    ...
    if (lowerBound > upperBound) {
      throw new Error(
        'infeasible problem detected: lower bound strictly greater than upper bound'
      );
    }
    if (lowerBound < 0) {
      throw new Error(
        'incorrect problem detected: lower bound strictly lower than 0'
      );
    }
    if (upperBound > 1) {
      throw new Error(
        'incorrect problem detected: upper bound strictly greater than 1'
      );
    }
  ...
  if (sumLowerBounds > 1 || sumUpperBounds < 1) {
    throw new Error(
      'infeasible problem detected: the restricted simplex is empty'
    );
  }
...
function simplexGridSampler_(n, k, useArrayCopy) {
  ...
  this.x =
    typeof Float64Array === 'function' ? new Float64Array(n) : new Array(n); // the coordinates of a point being sampled
...
function simplexRandomSampler_(n, l, u, rnd) {
  ...
  this.u =
    typeof Float64Array === 'function'
      ...
      : new Array(n - 1); // the coordinates of a random point in the unit hypercube of R^n-1
  ...
  if (this.lowerBounds || this.upperBounds) {
    ...
    if (this.sumLowerBounds == 1 || this.sumUpperBounds == 1) {
      ...
      for (var i = 0; i < this.n; ++i) {
        ...
        var updatedLowerBound = Math.max(
          lowerBound,
          upperBound + 1 - this.sumUpperBounds
        );
        var updatedUpperBound = Math.min(
          upperBound,
          lowerBound + 1 - this.sumLowerBounds
        );
      ...
      this.upperStarBounds =
        typeof Float64Array === 'function' ? new Float64Array(n) : new Array(n);
      this.runningSumUpperStarBounds =
        typeof Float64Array === 'function' ? new Float64Array(n) : new Array(n);

      var sumUpperStarBounds = 0;
      for (var i = 0; i < this.n; ++i) {
        this.upperStarBounds[i] =
          (this.upperBounds[i] - this.lowerBounds[i]) /
          (1 - this.sumLowerBounds);
  ...
  this.sample_with_bounds = function () {
    ...
    for (var k = n; k >= 2; --k) {
      ...
      var d_k = Math.max(
        0,
        1 - this.runningSumUpperStarBounds[k - 2] / delta_k
      );
      ...
      var y_k =
        ...
        (1 -
          Math.pow(
            u_k * Math.pow(1 - phi_k, k - 1) +
              (1 - u_k) * Math.pow(1 - d_k, k - 1),
            1 / (k - 1)
          ));
    ...
    if (k == 1) {
      // In case the main loop above exited with delta not numerically null
...
function simplexRationalRounding_(x, r) {
  ...
  xPartsWithIndexes.sort(function (a, b) {
    if (b[1] < a[1]) {
      ...
      // coordinates have equal fractional parts
      return b[0] - a[0];
    }
  });
