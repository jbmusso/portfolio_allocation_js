...
function ftca_(correlationMatrix, threshold) {
  ...
  while (unassignedElementsIdx.length != 0) {
    ...
    if (unassignedElementsIdx.length === 1) {
      ...
      var subCorrMat = correlationMatrix.submatrix(
        unassignedElementsIdx,
        unassignedElementsIdx
      );
      ...
      if (correlationMatrix.getValueAt(hc, lc) > threshold) {
        ...
        for (var i = 0; i < unassignedElementsIdx.length; ++i) {
          if (unassignedElementsIdx[i] !== null) {
            // Skip assigned elements (HC and LC)
            var avgHcLcAssetCorrelation =
              (correlationMatrix.getValueAt(unassignedElementsIdx[i], hc) +
                correlationMatrix.getValueAt(unassignedElementsIdx[i], lc)) /
              2;
      ...
      // Else
        ...
        if (hc !== lc) {
          ...
          for (var i = 0; i < unassignedElementsIdx.length; ++i) {
            if (unassignedElementsIdx[i] !== null) {
              // Skip assigned assets (HC with its correlated assets, and LC)
              if (
                correlationMatrix.getValueAt(unassignedElementsIdx[i], lc) >
                threshold
              ) {
...
function norminv_(p) {
  ...
  if (Math.abs(q) <= 0.425) {
    ...
    var r = 0.180625 - q * q;
    var num_ppnd16 =
      ((((((r * 2.5090809287301226727e3 + 3.3430575583588128105e4) * r +
    ...
    var denom_ppnd16 =
      ((((((r * 5.226495278852854561e3 + 2.8729085735721942674e4) * r +
    ...
    if (r <= 5) {
      ...
      r = r - 1.6;
      var num_ppnd16 =
        ((((((r * 7.7454501427834140764e-4 + 2.27238449892691845833e-2) * r +
