...
self.meanVarianceOptimizationWeights = function (mu, sigma, opt) {
  ...
  if (optimizationMethod === 'targetReturn' && targetReturn === undefined) {
    throw new Error(
      'missing mean-variance optimization method return constraint'
    );
  }
  ...
  if (
    ...
    throw new Error(
      'missing mean-variance optimization method volatility constraint'
    );
  }
  ...
  if (
    optimizationMethod === 'maximumTargetVolatility' &&
    maxTargetVolatility === undefined
  ) {
    throw new Error(
      'missing mean-variance optimization method maximum volatility constraint'
    );
  }
  ...
  if (optimizationMethod == 'targetReturn') {
    efficientPortfolio = computeTargetReturnEfficientPortfolio_(
      mu,
      targetReturn,
      cornerPortfolios
    );
    ...
    efficientPortfolio = computeTargetVolatilityEfficientPortfolio_(
      sigma,
      targetVolatility,
      cornerPortfolios
    );
    ...
    efficientPortfolio = computeMinimumVarianceEfficientPortfolio_(
      cornerPortfolios
    );
    ...
    efficientPortfolio = computeMaximumTargetVolatilityEfficientPortfolio_(
      mu,
      sigma,
      maxTargetVolatility,
      cornerPortfolios,
...
self.maximumSharpeRatioWeights = function (mu, sigma, rf, opt) {
  ...
  function computeVolatility_(sigma, weights) {
    return Math.sqrt(
      Matrix_.vectorDotProduct(Matrix_.xy(sigma, weights), weights)
    );
  }
  ...
  function computeMaximumSharpeRatioCornerPortfolio_(
    mu,
    sigma,
    rf,
    cornerPortfolios
    ...
    while (idx_min - idx_max != 1) {
      ...
      var sharpeRatio_middle = computeSharpeRatio_(
        mu,
        sigma,
        rf,
        weights_middle
      ...
      var sharpeRatio_middle_p = computeSharpeRatio_(
        mu,
        sigma,
        rf,
        weights_middle_p
  ...
  function computeLocalMaximumSharpeRatioEfficientPortfolio_(
    mu,
    sigma,
    rf,
    idx_min,
    ...
    var variance_cross = Matrix_.vectorDotProduct(
      Matrix_.xy(sigma, weights_min),
      weights_max
    ); // <Sigma*w_min/w_max>
    ...
    if (disc < 0) {
      throw new Error(
        'internal error, the covariance matrix might not be semi-definite positive'
      );
    }
    ...
    var candidateSharpeRatios = [
      [weights_min, sr_min],
      [weights_max, sr_max],
    ]; // t = 0 and t = 1 portfolios are always present

    if (t1 > 0 && t1 < 1) {
      // t1 belongs to ]0,1[
      var weights_t1 = Matrix_.fill(weights_min.nbRows, 1, function (i, j) {
        return (
          t1 * weights_min.getValue(i, 1) +
          (1 - t1) * weights_max.getValue(i, 1)
        );
      });
    ...
    if (t2 > 0 && t2 < 1) {
      // t2 belongs to ]0,1[
      var weights_t2 = Matrix_.fill(weights_min.nbRows, 1, function (i, j) {
        return (
          t2 * weights_min.getValue(i, 1) +
          (1 - t2) * weights_max.getValue(i, 1)
        );
      });
  ...
  if (volatility < eps) {
    ...
    // volatility.
    var efficientPortfolio = computeTargetVolatilityEfficientPortfolio_(
      sigma,
      eps,
      cornerPortfolios
    );
    if (efficientPortfolio.length == 0) {
      throw new Error(
        'no corner portfolio with a strictly positive volatility: the covariance matrix might not be semi-definite positive'
      );
    }
  ...
  if (ret < rf + eps) {
    ...
    // Compute the first efficient portfolio with a strictly positive
    // excess return.
    var efficientPortfolio = computeTargetReturnEfficientPortfolio_(
      mu,
      rf + eps,
      cornerPortfolios
    );
    if (efficientPortfolio.length == 0) {
      throw new Error(
        'no corner portfolio with a strictly positive excess return'
      );
    }
    var efficientPortfolioWeights = efficientPortfolio[0];
    var cornerPortfolioIndexMin = efficientPortfolio[1];
  ...
  var cornerPortfolio = computeMaximumSharpeRatioCornerPortfolio_(
    mu,
    sigma,
    rf,
    cornerPortfolios
  ...
  if (idx_min <= cornerPortfolios.length - 1) {
    ...
    var weights_min_middle = computeLocalMaximumSharpeRatioEfficientPortfolio_(
      mu,
      sigma,
      rf,
      idx_min,
  ...
  if (idx_max >= 0) {
    ...
    var weights_middle_max = computeLocalMaximumSharpeRatioEfficientPortfolio_(
      mu,
      sigma,
      rf,
      idx_middle,
...
self.meanVarianceEfficientFrontierPortfolios = function (mu, sigma, opt) {
  ...
  if (cornerPortfolios.length == 0) {
    throw new Error(
      'efficient frontier made of no corner portfolios: internal error'
    );
  } else if (cornerPortfolios.length == 1) {
    if (nbPortfolios != 1) {
      throw new Error(
        'efficient frontier made of only one corner portfolio: only one efficient portfolio can be computed'
      );
    } else {
      var portfolioWeights = cornerPortfolios[0][0];
      var portfolioReturn = Matrix_.vectorDotProduct(mu, portfolioWeights);
      var portfolioVolatility = Math.sqrt(
        Matrix_.vectorDotProduct(
          Matrix_.xy(sigma, portfolioWeights),
          portfolioWeights
        )
      );

      return [
        [portfolioWeights.toArray(), portfolioReturn, portfolioVolatility],
      ];
    }
  } else {
    // cornerPortfolios.length >= 2
    if (nbPortfolios <= 1) {
      throw new Error(
        'efficient frontier made of several corner portfolios: at least two efficient portfolios must be computed'
      );
    }
  }
  ...
  // minimum variance portfolio).
  {
    ...
    var portfolioReturn = Matrix_.vectorDotProduct(
      mu,
      minimumVariancePortfolioWeights
    );
    var portfolioVolatility = Math.sqrt(
      Matrix_.vectorDotProduct(
        Matrix_.xy(sigma, minimumVariancePortfolioWeights),
        minimumVariancePortfolioWeights
      )
    );

    efficientFrontier[0] = [
      minimumVariancePortfolioWeights.toArray(),
      portfolioReturn,
      portfolioVolatility,
    ];
  }
  ...
  for (var i = 1; i < nbPortfolios - 1; ++i) {
    ...
    var portfolioWeights = Matrix_.fill(nbAssets, 1, function (i, j) {
  ...
  lambda_i_max_idx = lambda_i_min_idx - 1;
  {
    ...
    var portfolioVolatility = Math.sqrt(
      Matrix_.vectorDotProduct(
        Matrix_.xy(sigma, maximumReturnPortfolioWeights),
        maximumReturnPortfolioWeights
      )
    );

    efficientFrontier[nbPortfolios - 1] = [
      maximumReturnPortfolioWeights.toArray(),
      portfolioReturn,
      portfolioVolatility,
    ];
  }
...
self.meanVarianceCornerPortfolios = function (mu, sigma, opt) {
  ...
  for (var i = 0; i < cornerPortfolios.length; ++i) {
    ...
    efficientFrontier[cornerPortfolios.length - 1 - i] = [
      portfolioWeights.toArray(),
      portfolioReturn,
      portfolioVolatility,
    ];
...
function computeTargetReturnEfficientPortfolio_(
  ...
  function computeEnclosingCornerPortfolios_(
    targetReturn,
    mu,
    cornerPortfolios
  ) {
    ...
    while (idx_min - idx_max != 1) {
      ...
      if (return_middle - targetReturn > eps) {
        ...
        // the target return is exactly attained on the idx_middle-th corner portfolio
    ...
    return [
      [idx_min, weights_min, return_min],
      [idx_max, weights_max, return_max],
    ];
  }
  ...
  var enclosingCornerPortfolios = computeEnclosingCornerPortfolios_(
    targetReturn,
    mu,
    cornerPortfolios
  );
  ...
  if (enclosingCornerPortfolios.length == 0) {
    ...
    var weights = Matrix_.fill(weights_min.nbRows, 1, function (i, j) {
      return (
        t * weights_min.getValue(i, 1) + (1 - t) * weights_max.getValue(i, 1)
      );
    });
...
function computeTargetVolatilityEfficientPortfolio_(
  ...
  function computeEnclosingCornerPortfolios_(
    ...
    // return immediately.
    if (
      targetVolatility - volatility_max > eps ||
      -eps > targetVolatility - volatility_min
    ) {
      return [];
    }
    ...
    while (idx_min - idx_max != 1) {
      ...
      if (volatility_middle - targetVolatility > eps) {
        ...
        // the target volatility is exactly attained on the idx_middle-th corner portfolio
    ...
    // the associated function values.
    return [
      [idx_min, weights_min, volatility_min],
      [idx_max, weights_max, volatility_max],
    ];
  }
...
function computeMaximumTargetVolatilityEfficientPortfolio_(
  ...
  if (maxVolatility > highestVolatility + eps) {
    ...
    // frontier.
    var lowestVolatilityWeights =
      cornerPortfolios[cornerPortfolios.length - 1][0];
    ...
    if (maxVolatility < lowestVolatility - eps) {
      ...
      // of index (nbAssets + 1).
      var newMu = Matrix_.fill(nbAssets + 1, 1, function (i, j) {
        if (i <= nbAssets) {
      ...
      var newSigma = Matrix_.fill(nbAssets + 1, nbAssets + 1, function (i, j) {
      ...
      if (newCornerPortfolios.length === 1) {
        ...
        if (newCornerPortfolios[0][0].getValue(nbAssets + 1, 1) != 1) {
          // this case should never occur, as per description above
          throw new Error('internal error');
        }
      } else {
        var efficientPortfolio = computeTargetVolatilityEfficientPortfolio_(
        ...
        if (efficientPortfolio.length === 0) {
          // this case should never occur, since the new efficient frontier includes a 0 volatility portfolio and at least 2 corner portfolios
          throw new Error('internal error');
        } else {
      ...
      // risk free asset.
      weights = Matrix_.fill(nbAssets, 1, function (i, j) {
        if (i <= nbAssets) {
    ...
    // the input efficient frontier.
      ...
      if (efficientPortfolio.length === 0) {
        // this case should never occur, since maxVolatility belongs to [lowestVolatility, highestVolatility]
        throw new Error('internal error');
      } else {
...
function computeCornerPortfolios_(mu, sigma, opt) {
  ...
  function variablesStatusManager_(nbAssets, nbEqualityConstraints) {
    ...
    this.setLambdasIn = function () {
      for (
        var i = this.nbAssets + 1;
        i <= this.nbAssets + this.nbEqualityConstraints;
        ++i
      ) {
        this.varIn.set(i);
        this.varOut.unset(i);
    ...
    this.isLambda = function (idx) {
      return (
        idx >= this.nbAssets + 1 &&
        idx <= this.nbAssets + this.nbEqualityConstraints
      );
    };
  ...
  function computeMaxReturnPortfolio_(mu, lowerBounds, upperBounds) {
    ...
    for (var i = 0; i < nbAssets; ++i) {
      ...
      var inc_weight_asset = Math.min(
        upperBounds.getValue(idx_i, 1) - weight_asset,
        delta_sum_weights
      );
  ...
  var variablesStatusManager = new variablesStatusManager_(
    nbAssets,
  ...
  var M = Matrix_.fill(
    nbAssets + nbEqualityConstraints,
    nbAssets + nbEqualityConstraints,
    function (i, j) {
      if (i <= nbAssets && j <= nbAssets) {
  ...
  var T = Matrix_.atxy(
    -1,
    Ai,
    Matrix_.xy(sigma.submatrix(assetsInIdx, assetsInIdx), Ai)
  );
  ...
  for (var i = 1; i <= assetsInIdx.length; ++i) {
    ...
    for (var j = 1; j <= assetsOutIdx.length; ++j) {
      ...
      b_bar_in_idx_i -=
        M.getValue(in_idx_i, out_idx_j) *
        currentCornerPortfolioWeights.getValue(out_idx_j, 1);
    }
  ...
  while (true) {
    ...
    if (iter >= 2) {
      if (lambda_out >= lambda_in) {
        // an asset IN goes OUT
        ...
        for (var i = 1; i <= variablesInIdx.length; ++i) {
          ...
          for (var j = 1; j <= variablesInIdx.length; ++j) {
            ...
            Mi.setValue(
              in_idx_i,
              in_idx_j,
              Mi.getValue(in_idx_i, in_idx_j) -
                (Mi.getValue(in_idx_i, idx_out) *
                  Mi.getValue(idx_out, in_idx_j)) /
                  Mi_out_idx_out_idx
            );
          }
        }
        ...
        for (var i = 1; i <= variablesInIdx.length; ++i) {
          ...
          b_bar.setValue(
            in_idx_i,
            1,
            b_bar.getValue(in_idx_i, 1) -
              M.getValue(in_idx_i, idx_out) *
                currentCornerPortfolioWeights.getValue(idx_out, 1)
          );
        }
      } else {
        // an asset OUT goes IN
        ...
        for (var i = 1; i <= variablesInIdx.length; ++i) {
          ...
          for (var j = 1; j <= variablesInIdx.length; ++j) {
            ...
            xi_in_idx_i +=
              Mi.getValue(in_idx_i, in_idx_j) * M.getValue(in_idx_j, idx_in);
          }
        ...
        for (var i = 1; i <= variablesInIdx.length; ++i) {
          ...
          for (var j = 1; j <= variablesInIdx.length; ++j) {
            ...
            Mi.setValue(
              ...
              in_idx_j,
              Mi.getValue(in_idx_i, in_idx_j) +
                (xi.getValue(in_idx_i, 1) * xi.getValue(in_idx_j, 1)) / xi_j
            );
          }
        ...
        for (var i = 1; i <= variablesInIdx.length; ++i) {
          ...
          b_bar.setValue(
            ...
            1,
            b_bar.getValue(in_idx_i, 1) +
              M.getValue(in_idx_i, idx_in) *
                currentCornerPortfolioWeights.getValue(idx_in, 1)
          );
        }
    ...
    for (var i = 1; i <= variablesInIdx.length; ++i) {
      ...
      if (variablesStatusManager.isAsset(in_idx_i)) {
        currentCornerPortfolioWeights.setValue(
          in_idx_i,
          1,
    ...
    cornerPortfoliosWeights.push([
      new Matrix_(currentCornerPortfolioWeights),
      lambda_e,
    ]);
