...
self.clusterRiskParityWeights = function (sigma, opt) {
  ...
  for (var i = 0; i < nbClusters; ++i) {
    ...
    var assetsIndexes = clusters[i].slice().sort(function (a, b) {
      return a - b;
    });
    ...
    for (var j = 0; j < assetsWeights.length; ++j) {
      assetsToClustersWeights.setValueAt(
        i + 1,
        assetsIndexes[j],
        assetsWeights[j]
      );
  ...
  var clustersSigma = Matrix_.xy(
    assetsToClustersWeights,
    Matrix_.xy(sigma, assetsToClustersWeightsT)
  );
  ...
  var weights = Matrix_.xy(
    assetsToClustersWeightsT,
    new Matrix_(clustersWeights)
  );
